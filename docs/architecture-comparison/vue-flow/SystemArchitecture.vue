...
const DatabaseNode = (props: NodeProps) => {
  const label = computed(() => (props.data as { label: string }).label)
  const stroke = computed(() => (props.selected ? 3 : 2))
  return () => (
    <div style={{ position: 'relative' }}>
      <Handle type="target" position={Position.Top} />
      <svg width="120" height="80" viewBox="0 0 120 80">
        <ellipse cx="60" cy="12" rx="50" ry="8" fill="#ffe6cc" stroke="#d79b00" stroke-width={stroke.value} />
        <path
        ...
        />
        <ellipse cx="60" cy="12" rx="50" ry="8" fill="none" stroke="#d79b00" stroke-width="1" opacity="0.5" />
        <ellipse cx="60" cy="35" rx="50" ry="8" fill="none" stroke="#d79b00" stroke-width="1" opacity="0.4" />
        <text x="60" y="44" text-anchor="middle" font-size="11" font-weight="600" fill="#5d3a00">
          {label.value}
        </text>
      </svg>
...
const CloudNode = (props: NodeProps) => {
  const label = computed(() => (props.data as { label: string }).label)
  const stroke = computed(() => (props.selected ? 3 : 2))
  return () => (
    <div style={{ position: 'relative' }}>
      ...
      <svg width="140" height="70" viewBox="0 0 140 70">
        <path
          ...
          stroke-width={stroke.value}
        />
        <text x="70" y="40" text-anchor="middle" font-size="11" font-weight="600" fill="#3d2c4a">
          {label.value}
        </text>
      </svg>
...
// ─── Graph ────────────────────────────────────────────────────────────────────
const nodes: Node[] = [
  { id: 'fe', type: 'default', position: { x: 240, y: 0 }, data: { label: 'Frontend\nVue 3 + Vite\n+ Pinia + Element Plus' } },
  { id: 'be', type: 'default', position: { x: 240, y: 180 }, data: { label: 'Backend\nExpress 5 + Prisma + Pino' } },
  { id: 'db', type: 'database', position: { x: 0, y: 360 }, data: { label: 'MySQL 8' } },
  { id: 'rd', type: 'database', position: { x: 200, y: 360 }, data: { label: 'Redis 7' } },
  { id: 'vc', type: 'database', position: { x: 400, y: 360 }, data: { label: 'Chroma' } },
  { id: 'llm', type: 'cloud', position: { x: 560, y: 180 }, data: { label: 'DeepSeek\ndeepseek-v4-flash' } },
  { id: 'emb', type: 'cloud', position: { x: 560, y: 360 }, data: { label: 'bge-small-zh\nEmbedding' } },
]
